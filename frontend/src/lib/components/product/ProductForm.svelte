<script lang="ts">
  ...
  const inputClass = 'w-full rounded-lg border border-zinc-800/80 bg-[#09090b] px-3 py-2.5 text-sm text-zinc-200 placeholder-zinc-700 focus:border-zinc-700 focus:bg-[#050507] focus:outline-none transition-all fld';
...
<form on:submit|preventDefault={submit} class="flex w-[65%] flex-col justify-between bg-[#121214]">
  ...
  <div class="flex-1 overflow-y-auto px-8 py-2 space-y-5 custom-scrollbar max-h-[64vh]">
    <div>
      <div class="flex items-center justify-between mb-2">
        <span class="block text-[11px] font-bold text-zinc-500 uppercase tracking-widest">Nome do Produto</span>
    ...
    <div>
      ...
      <input
        ...
        required
      />
      <p class="mt-2 text-[11px] text-zinc-500">Gerado automaticamente a partir do Nome PT, mas pode ser editado manualmente.</p>
    </div>
    ...
    <div>
      <span class="block text-[11px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Categoria</span>
      {#if activeLang === 'pt'}
    ...
    <div>
      ...
      {#if activeLang === 'pt'}
        <textarea rows="4" class="{inputClass} resize-none" placeholder="Crie, aplique e corrija avaliações..." bind:value={formData.description_pt}></textarea>
      {:else}
        <textarea rows="4" class="{inputClass} resize-none" placeholder="Create, deliver and grade assessments..." bind:value={formData.description_en}></textarea>
      {/if}
    </div>
  ...
  <div class="flex items-center justify-between border-t border-zinc-800/80 px-8 py-4 bg-[#121214]">
    <span class="text-[10px] text-zinc-600 font-medium uppercase tracking-wider">▲ PT e EN são gravados separadamente</span>
