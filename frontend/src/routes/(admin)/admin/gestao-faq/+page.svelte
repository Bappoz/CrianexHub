<script lang="ts">
  ...
  // ── Permissions ──────────────────────────────────────────
  const _faqUser = $derived($page.data?.adminUser);
  const canEditFaq = $derived(
    _faqUser?.role === 'owner' || (_faqUser?.permissions?.faq ?? []).includes('e')
  );
  const canAdminFaq = $derived(
    _faqUser?.role === 'owner' || (_faqUser?.permissions?.faq ?? []).includes('a')
  );

  // ── Topbar action ────────────────────────────────────────
  onMount(() => {
    const u = get(page).data?.adminUser;
    const canEdit = u?.role === 'owner' || (u?.permissions?.faq ?? []).includes('e');
    const canAdmin = u?.role === 'owner' || (u?.permissions?.faq ?? []).includes('a');
    const actions: { label: string; onClick: () => void }[] = [];
    if (canAdmin) actions.push({ label: 'Categorias', onClick: openCatManager });
    if (canEdit) actions.push({ label: '+ Novo artigo', onClick: openCreateModal });
          ...
          <div class="row-actions" style="position:relative">
            ...
            {#if activeMenuId === article.id}
              <div class="row-menu-pop" role="menu">
                ...
                {/if}
                {#if canAdminFaq}
                  {#if canEditFaq}<div class="sep"></div>{/if}
                  <button
                    role="menuitem"
                    class="danger"
